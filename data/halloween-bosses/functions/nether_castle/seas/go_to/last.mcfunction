teleport @a[tag=SvlyWrpd] @e[tag=svly_last_entrance,limit=1]

title @a[tag=SvlyWrpd] title {"text":"The Warped Portals","color":"dark_aqua","bold":true}
title @a[tag=SvlyWrpd] subtitle {"text":"When all other lights go out","color":"aqua"}
tellraw @a[tag=SvlyWrpd] {"text":"When all other lights go out...","color":"aqua"}
