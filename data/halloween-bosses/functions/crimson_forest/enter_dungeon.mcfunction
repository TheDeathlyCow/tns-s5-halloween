scoreboard players set enteredCrimson entrdDng 1
# say stuff??
...
execute as @e[type=minecraft:armor_stand,tag=dir_chest] at @s run setblock ~ ~ ~ chest[facing=north]{Items:[{Slot:13b,id:"minecraft:compass",Count:1b,tag:{display:{Name:'{"bold":true,"italic":false,"color":"aqua","text":"Direction"}'},direction_compass:1b}}]} replace
execute as @e[type=minecraft:armor_stand,tag=time_chest] at @s run setblock ~ ~ ~ chest[facing=south]{Items:[{Slot:13b,id:"minecraft:clock",Count:1b,tag:{display:{Name:'{"bold":true,"italic":false,"color":"gold","text":"Time"}'},time_clock:1b}}]} replace
